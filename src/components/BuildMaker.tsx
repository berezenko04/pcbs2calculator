...
import type { CPU, GPU, RAM, Motherboard, PSU, StorageDrive, Case, Cooler, LevelSettings, ScoreResult } from '@/lib/types'
import InputCard from '@/components/ui/InputCard'
import SelectCard from '@/components/ui/SelectCard'
import ToggleSwitch from '@/components/ui/ToggleSwitch'
import BuildMakerResultCard from './BuildMakerResultCard'
...
export interface BuildResultSimple {
  cpu: CPU; gpu: GPU; ram: RAM; ramQty: number; gpuQty: number
  totalPrice: number; cpuScore: number; gpuScore: number; totalScore: number
  rank: string; totalTdp: number
}

export interface BuildResultFull {
  cpu: CPU; gpu: GPU; ram: RAM; ramQty: number; gpuQty: number
  motherboard: Motherboard; psu: PSU; storage: StorageDrive; case: Case; cooler: Cooler
  totalPrice: number; cpuScore: number; gpuScore: number; totalScore: number
  rank: string; totalTdp: number
}

export type BuildResult = BuildResultSimple | BuildResultFull

interface Props {
  cpus: CPU[]; gpus: GPU[]; rams: RAM[]; motherboards: Motherboard[]; psus: PSU[]
  storageDrives: StorageDrive[]; cases: Case[]; coolers: Cooler[]
  levelSettings: LevelSettings | null; gameVersion?: string
}

function selectDiverseBuilds<T extends BuildResult>(builds: T[], maxCount: number): T[] {
  ...
  for (let i = 1; i < sorted.length && selected.length < maxCount; i++) {
    const b = sorted[i]
    const bCpuId = b.cpu.id
    const bGpuId = b.gpu.id
    if (usedCpu.has(bCpuId) || usedGpu.has(bGpuId)) continue
    let ok = true
    for (const s of selected) {
      let diff = 0
      if (bCpuId !== s.cpu.id) diff++
      if (bGpuId !== s.gpu.id) diff++
...
export default function BuildMaker({ cpus, gpus, rams, motherboards, psus, storageDrives, cases, coolers, levelSettings, gameVersion }: Props) {
  ...
  const storageSizeOptions = useMemo(() => {
    ...
    const sizes = storageDrives.reduce<number[]>((acc, s) => {
      if (s.size_gb) acc.push(Number(s.size_gb))
      return acc
    }, [])
    return [...new Set(sizes)].sort((a, b) => a - b).reduce<string[]>((acc, v) => {
      const f = formatSizeGb(v)
  ...
  const ramSizes = useMemo(() => {
    const sizes = new Set(rams.map(r => r.total_size_gb))
    sizes.add(64)
  ...
  const doSimpleSearch = () => {
    ...
    const found = new Map<string, BuildResultSimple>()
      ...
      for (const gpu of gpuCandidates) {
        for (const ram of ramCandidates) {
          ...
          if (!existing || score.totalScore > existing.totalScore) {
            found.set(key, { cpu, gpu, ram, ramQty, gpuQty, totalPrice, cpuScore: score.cpuScore, gpuScore: score.gpuScore, totalScore: score.totalScore, rank: score.rank, totalTdp })
          }
        }
      }
    }

    setResults(selectDiverseBuilds([...found.values()], 10))
    setSearched(true)
  }
    ...
    <div>
      <div className="flex justify-center mb-6">
        <div className="inline-flex bg-white/80 dark:bg-gray-800/60 backdrop-blur-sm border border-slate-200 dark:border-gray-700 rounded-xl p-1 shadow-sm">
          <button type="button" onClick={() => { setMode('simple'); setResults([]); setSearched(false) }}
            className={'px-5 py-2 rounded-lg text-sm font-medium transition-colors ' + (mode === 'simple' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400')}
          >{t('bm_simple')}</button>
          <button type="button" onClick={() => { setMode('full'); setResults([]); setSearched(false) }}
            className={'px-5 py-2 rounded-lg text-sm font-medium transition-colors ' + (mode === 'full' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400')}
          >{t('bm_full_build')}</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        ...
        {mode === 'full' && <SelectCard icon={<Cpu className="h-4 w-4 text-blue-500" />} label={t('bm_socket')} value={socket} onChange={handleSocketChange} options={[...SOCKETS]} anyLabel={t('bm_any')} />}
        {mode === 'full' && <SelectCard icon={<Layers className="h-4 w-4 text-purple-500" />} label={t('bm_cpu_brand')} value={cpuBrand} onChange={handleCpuBrandChange} options={cpuBrandOptions} anyLabel={t('bm_any')} />}
        {mode === 'full' && <SelectCard icon={<Gpu className="h-4 w-4 text-green-500" />} label={t('bm_gpu_brand')} value={gpuBrand} onChange={setGpuBrand} options={['AMD', 'NVIDIA']} anyLabel={t('bm_any')} />}
        {mode === 'full' && <SelectCard icon={<MemoryStick className="h-4 w-4 text-purple-500" />} label={t('bm_min_ram')} value={minRamGb ? minRamGb + 'GB' : ''} onChange={(v) => setMinRamGb(Number(v.replace('GB', '')))} options={ramSizes.map(s => s + 'GB')} anyLabel={t('bm_any')} />}
        {mode === 'full' && <SelectCard icon={<Box className="h-4 w-4 text-orange-500" />} label={t('bm_mobo_size')} value={moboSize} onChange={setMoboSize} options={[...CASE_SIZES]} anyLabel={t('bm_any')} />}
      ...
      <div className="text-center mb-8">
        ...
        {budgetError && (
          <div className="mt-3 text-sm text-rose-500 dark:text-rose-400 font-medium animate-pulse">{budgetError}</div>
        )}
      </div>
        ...
        <div>
          {results.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              {results.map((r, i) => (
                <BuildMakerResultCard key={r.cpu.id + '|' + r.gpu.id + '|' + r.ram.id + '|' + i} result={r} index={i} />
              ))}
            </div>
          )}
        </div>
